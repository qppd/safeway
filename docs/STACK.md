...
| Board (camera) | **AI Thinker ESP32-CAM** on MB programmer (CH340) | Camera sketch `safeway-cam` — `/capture` JPEG, `/stream` MJPEG, microSD backup |
...
| Plate recognition (path A) | **OpenALPR** agent (`alpr -c ph`) | Preferred — purpose-built ANPR with PH region config |
| Plate recognition (fallback B) | **pytesseract** + **OpenCV** (`opencv-python`) | Pure-Python: grayscale → bilateral filter → Canny → Tesseract PSM 7 |
...
| KY-008 laser TX + photoresistor receiver | Break-beam presence confirmation across the lane |
...
- **Graceful upgrades** — SQLite → PostgreSQL, local `uploads/` → object storage, OpenALPR → cloud OCR: each swap is isolated to one module.
